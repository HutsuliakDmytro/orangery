...
- [x] TXT, Markdown, HTML — import + export (HTML: колір, шрифт, розмір, підсвітка, вирівнювання, зображення, таблиці; Markdown: вкладені списки, таблиці, зображення)
- [x] RTF — import + export: форматування тексту (колір, шрифт, розмір, підсвітка), вирівнювання, списки з вкладеністю, таблиці, зображення
      — зображення лише PNG і JPEG: інші кодування RTF (метафайли, DIB) потребують растеризатора, якого в застосунку немає; такі картинки лишаються з попередженням
- [x] ODT — import + export через той самий підхід (пакет + passthrough), мінімум: текст, стилі, списки, таблиці
      — текст, заголовки, стилі, посилання, списки, таблиці, зображення, колір/шрифт/розмір/підсвітка та вирівнювання; тип списку читається з `text:list-style` у `styles.xml` і `content.xml`
...
## Фаза 4.5 — Глибина DOCX (редагування рівня Word)

Мета: те, без чого курсова, диплом чи звіт не оформлюються — усе через реальні конструкції OOXML, не через візуальну імітацію.

### 4.5.1 Дешеве й фундаментне

- [x] Контроль розбиття абзаців: `w:keepNext`, `w:keepLines`, `w:pageBreakBefore`, `w:widowControl` — читання, запис, UI
      — «з нової сторінки» ще й видно в редакторі: `PageGaps` починає з такого абзацу новий аркуш. Три інші виявляють себе тільки у справжній посторінковій верстці, якої в редакторі немає — вони зберігаються, бо Word і друк за ними верстають
- [x] Копіювання формату (format painter), `Mod+Option+C` / `Mod+Option+V`, підхоплює марки та властивості абзацу
      — тип блока не переноситься: перетворити абзац на заголовок це зміна структури, а не вигляду. Посилання й збережені властивості run теж не фарбуються
- [x] Автозаміна при наборі: лапки за мовою документа (укр. «», англ. “”), `--` → `—`, `...` → `…`, нерозривний пробіл перед тире; вимикається в налаштуваннях
      — мова визначається за найближчою літерою в тексті, а не за мовою інтерфейсу. Одинарна лапка перетворюється лише всередині слова: в українській це апостроф, а не пунктуація

### 4.5.2 Нумерація і зміст

- [x] Багаторівнева нумерація заголовків (`1.`, `1.1.`, `1.1.1.`): `w:numPr` у стилях Heading + `w:lvlText` у numbering.xml, вибір схеми, увімкнення/вимкнення
      — дві схеми: `1. / 1.1. / 1.1.1.` і галерея `I. / A. / 1. / a) / i)`. Номер обчислюється з документа й малюється декорацією, у текст не пишеться
- [x] Автозміст: кнопка, що будує зміст із заголовків разом з їхніми номерами з попереднього пункту; оновлення по кліку
      — клік по рядку змісту переходить до заголовка. В DOCX пишеться полем Word (`TOC`), тож «Оновити зміст» у Word працює
- [x] Автонумерація сторінок: кнопка вставки номера в колонтитул + налаштування — формат (`1`, `i`, `A`), з якого числа починати, «інакше на першій сторінці»
      — положення задається тим, у який колонтитул поставлено `{page}`; окремий колонтитул першої сторінки (не просто її відсутність) приїде з розділами у 4.5.3
- [x] Позиції табуляції `w:tabs` із заповнювачем (`w:leader="dot"`), маркери на лінійці
      — ширина табуляції вимірюється після верстки й повертається в декорацію; праве вирівнювання враховує ширину тексту після табуляції, тож рядок змісту стає на місце. Маркер: клік — вирівнювання, Alt-клік — заповнювач, подвійний клік — прибрати
- [x] Підписи з автонумерацією через поля `SEQ` («Рисунок 1.1», «Таблиця 2.3») + «Перелік ілюстрацій»
      — номер обчислюється з документа, у файл пишеться полем `SEQ` (зі `STYLEREF` для номера розділу), підрахунок у межах розділу вмикається разом із нумерацією заголовків. Перелік ілюстрацій і перелік таблиць — той самий вузол, що й зміст, із полем `TOC \c`

### 4.5.3 Розмітка сторінки

- [x] Розділи: кілька `w:sectPr` на документ, розрив розділу, окремі поля й орієнтація, `w:titlePg` (інший колонтитул на першій сторінці)
      — розрив видно як блок, параметри сторінки, лінійка й колонтитули діють на розділ під курсором, розрив починає нову сторінку, висота сторінки рахується за своїм розділом, перша сторінка має власні колонтитули (`w:type="first"`)
- [x] Колонки `w:cols` у межах розділу
      — кількість, проміжок і лінія між ними читаються, пишуться й задаються в параметрах сторінки; нерівні колонки зберігаються дослівно. На екрані колонки малюються для документа з одного розділу: CSS верстає їх на контейнері, а розділ — це послідовність блоків без власного контейнера

### 4.5.4 Стилі

- [x] Панель стилів: застосування, створення власного, «оновити за виділенням», стилі символів (`w:rStyle`)
      — стилі абзацу й символів читаються з документа, застосовуються, створюються з виділення й оновлюються за ним; `w:rStyle` тепер модель, а не втрата

### 4.5.5 Рецензування

Перенесено в межі релізу рішенням від 2026-09-17; у CLAUDE.md це було записано як out of scope для MVP.

- [~] Коментарі: `word/comments.xml`, бульбашки на полях, відповіді, розв'язання
      — читання, запис, панель збоку, додавання на виділення й видалення. Відповіді й розв'язання потребують `commentsExtended.xml` (w15) — окрема частина пакета
- [x] Track changes: `w:ins` / `w:del` / `w:rPrChange`, панель прийняти/відхилити, режим запису виправлень
      — вставки, видалення й зміни форматування читаються, показуються, записуються й пишуться у файл; прийняти/відхилити — на виділення або на весь документ; режим живе у файлі (`w:trackChanges`)

**DoD:** курсова з багаторівневими заголовками, автозмістом, нумерацією сторінок з другої, підписами до рисунків і альбомним додатком відкривається у Word без ремонту файлу й виглядає так само.
...
- [-] Tauri updater: endpoint, підпис оновлень, перевірка при старті
      — відкладено до **Update 2 — дистрибуція suite** (рішення від 2026-09-18). Оновлення й сторінка описують *набір* програм, а не одну з них: канал оновлень спільний, сторінка має три кнопки на апп, і зроблені під один апп вони переробляються, щойно виходить другий. Розділ Update 2 уже містить обидва пункти. Те, що вже зроблено, лишається на місці й чекає там
...
- [ ] Тег `v1.0.0`, GitHub Release
      — лендинг відокремлено й відкладено до Update 2: одна сторінка на набір, а не на апп
