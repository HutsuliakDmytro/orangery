...
- [~] Рядки/стовпці: вставити ✔ / видалити ✔ (скільки виділено, стільки й вставляється), з відкотом одним кроком ✔. Головне тут — не рух комірок, а те, що **кожна формула на аркуші** може вказувати на комірку, яка щойно переїхала: `adjustFormula` (`formulas.ts`) — інше питання, ніж `shiftFormula`, і відповідь інша (долар тут не рятує, бо він пришпилює до комірки, а переїхала саме комірка; посилання всередині видаленої смуги стає `#REF!`). Висоти рядків їдуть разом з рядками, але в історію не потрапляють — записано в коді, чому. Лишилось: приховати/показати, ширина/висота drag і auto-fit, group/outline
