...
- [~] Track changes: `w:ins` / `w:del` / `w:rPrChange`, панель прийняти/відхилити, режим запису виправлень
      — `w:ins` і `w:del` читаються, показуються й пишуться назад (текст видаленого йде як `w:delText`); прийняти/відхилити — на виділення або на весь документ; режим запису вмикається й живе у файлі (`w:trackChanges`). Лишився `w:rPrChange` — зміни форматування наразі не записуються як виправлення
