...
- [~] Розділи: кілька `w:sectPr` на документ, розрив розділу, окремі поля й орієнтація, `w:titlePg` (інший колонтитул на першій сторінці)
      — розрив видно як блок, параметри сторінки й лінійка діють на розділ під курсором, розрив починає нову сторінку, висота сторінки рахується за своїм розділом, перша сторінка має власні колонтитули (`w:type="first"`). Лишилися окремі колонтитули для кожного розділу — зараз редагуються колонтитули останнього
- [ ] Колонки `w:cols` у межах розділу
