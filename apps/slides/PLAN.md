...
- [~] SVG-рендерер слайда: фон (solid/gradient/picture, з master), фігури, текст (через ProseMirror у режимі read-only або статичний HTML), картинки, таблиці, групи
      — фігури, текст і групи малюються. Координати лишаються в EMU аж до viewBox: перетворення в пікселі десь по дорозі зробило б кожну позицію округленням. Текст живе в `foreignObject` **усередині** SVG, а не шаром поверх: `<text>` у SVG не переносить рядки, а viewBox тоді масштабує текст разом із фігурами без жодних вимірювань у JS. Перша спроба була шаром поверх зі `scale(calc(100% / …))` — CSS так не вміє, масштаб має бути безрозмірним. Пресети: таблиця з ~25 найуживаніших, решта малюється габаритним прямокутником — це видно на екрані й нічого не коштує файлу. Фон із майстра, картинки й таблиці ще не малюються
