...
- [~] Підписи з автонумерацією через поля `SEQ` («Рисунок 1.1», «Таблиця 2.3») + «Перелік ілюстрацій»
      — підписи готові: номер обчислюється з документа, у файл пишеться полем `SEQ` (зі `STYLEREF` для номера розділу), підрахунок у межах розділу вмикається разом із нумерацією заголовків. «Перелік ілюстрацій» ще попереду
