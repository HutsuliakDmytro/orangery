...
function!(ROW, "ROW", 0, Some(1), |arguments, context| {
    match arguments.first() {
        ...
        Some(Expr::Reference(reference)) => match first_cell(reference) {
            Some((row, _)) => Value::Number((row + 1) as f64),
            None => Value::Error(Error::Reference),
        },
        Some(_) => Value::Error(Error::Value),
    }
});
...
function!(COLUMN, "COLUMN", 0, Some(1), |arguments, context| {
    match arguments.first() {
        ...
        Some(Expr::Reference(reference)) => match first_cell(reference) {
            Some((_, column)) => Value::Number((column + 1) as f64),
            None => Value::Error(Error::Reference),
        },
        Some(_) => Value::Error(Error::Value),
    }
...
function!(ROWS, "ROWS", 1, Some(1), |arguments, context| {
    done(table(arguments.first(), context).map(|grid| Value::Number(grid.rows as f64)))
});
...
/// The top-left cell of a reference, which is what `ROW` and `COLUMN` answer.
fn first_cell(reference: &crate::reference::Reference) -> Option<(i64, i64)> {
    use crate::reference::ReferenceKind;

    match &reference.kind {
        ReferenceKind::Cell { row, column } => Some((row.index, column.index)),
        ReferenceKind::Range { from, .. } => Some((from.0.index, from.1.index)),
        ReferenceKind::Columns { from, .. } => Some((0, from.index)),
        ReferenceKind::Rows { from, .. } => Some((from.index, 0)),
    }
}
