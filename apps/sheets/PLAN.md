...
- [~] Undo/redo транзакційний ✔ (`document/history.ts`: крок — це список змін, скільки б комірок не зачепив; кожна зміна несе `before` і `after`, тож обидва напрямки — той самий обхід з іншим полем). Глибина 100 кроків, як в Excel. Delete по виділенню — один крок ✔. Стилі, які додало редагування, не відкочуються: невживаний `<xf>` легальний, а його видалення перенумерувало б усі наступні. Paste ще нема — коли буде, він уже потрапить в один крок
