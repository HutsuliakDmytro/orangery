...
- [~] Bullets: `buChar`, `buAutoNum`, `buBlip`, `buNone`, рівні 0–8 з lstStyle успадкуванням
      — усі чотири види читаються, разом із шрифтом, що несе гліф. `buNone` відрізняється від «нічого не сказано»: перше це рішення файлу, друге — успадкувати з рівня. Саме успадкування по ланцюгу shape → layout → master → defaultTextStyle ще попереду
