...
- [~] Розділи: кілька `w:sectPr` на документ, розрив розділу, окремі поля й орієнтація, `w:titlePg` (інший колонтитул на першій сторінці)
      — розрив видно як блок, параметри сторінки й лінійка діють на розділ під курсором, розрив починає нову сторінку, висота сторінки рахується за своїм розділом. Лишився власний колонтитул першої сторінки (потрібна друга частина пакета) і колонтитули по розділах
