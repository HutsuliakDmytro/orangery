 ...
 * inside an editor has to go through the editor. A spreadsheet has no editor
 * around the sheet, so this listens on the window — and steps aside when
 * something is being typed into, so that Mod+A in a cell being edited still
 * selects the text in it rather than the whole sheet.
 */
